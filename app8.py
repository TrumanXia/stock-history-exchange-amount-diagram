...
def main():
    ...
    for selected_date in selected_dates:
        ...
        if date_data:
            ...
            df['minute'] = df['minute'].astype(int)  # 确保minute列是整数类型
    ...
    if fig.data:
        # 获取所有minute值
        all_minutes = sorted(set().union(*[df['minute'].tolist() for df in date_dfs.values()]))
        
        start_minute = st.selectbox('选择开始时间点（分钟）', options=all_minutes, index=0)
        end_minute = st.selectbox('选择结束时间点（分钟）', options=all_minutes, index=len(all_minutes) - 1)
